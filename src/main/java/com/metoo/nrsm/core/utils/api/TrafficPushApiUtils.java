...
//    private final static String URL = "http://127.0.0.1:8930/api/nrsm/traffic/data";
        ...
        for (UnitVO unitVO : unitVos) {
            tasks.add(() -> {
                try {
                    ...
                    return trafficPushApiService.callThirdPartyApiT("http://59.52.34.196:6001/apisix/blade-ipv6/industryUnit", jindustryUnitRequest);
                } catch (Exception e) {
    ...
    public String partyApi(List<UnitVO> unitVos) {
        ...
        for (UnitVO unitVO : unitVos) {
            try {
                ...
                String apiResponse = trafficPushApiService.callThirdPartyApiTT(URL, jindustryUnitRequest);
                ...
                String errorMessage = String.format("推送单位 %s 时发生异常：%s", unitVO.getUnitName(), e.getMessage());
    ...
    public void monitorApi(List<UnitVO> unitVos) {
        try {
            for (UnitVO unitVo : unitVos) {
                trafficPushApiService.callDataLogToSelf(JSON.toJSONString(unitVo));
            }
