        ...
        try {
            if ("String".equalsIgnoreCase(fieldClassName)) {
                ...
                try {
                    ...
                    field.set(t, new SimpleDateFormat(dateTimeFormat.pattern()).parse(val));
                    System.out.println(dateTimeFormat.pattern());
                    System.out.println(new SimpleDateFormat(dateTimeFormat.pattern()).parse(val));
                } catch (Exception e) {
...
//        }
     ...
     * @param filePath 文件父路径（如：D:/doc/static/）
     ...
     * 获取文件
     *
     * @param filePath filePath 文件父路径（如：D:/doc/static/）
